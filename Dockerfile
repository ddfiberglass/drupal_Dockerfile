...
RUN cd /opt/bitnami/drupal && composer require drupal/ctools:^3.0 drupal/admin_toolbar drupal/field_permissions drupal/search_api drupal/search_api_solr drupal/facets drupal/address drupal/pathauto drupal/smart_date drupal/smtp drupal/honeypot --update-with-dependencies && composer upgrade

# && composer upgrade在RUN 最后面加上上面这个升级也可以运行，构建镜像。
